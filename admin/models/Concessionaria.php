...
class Concessionaria extends model
{
	...
	public function getServicoByConc($id)
	{
		...
		if ($sql->rowCount() > 0) {
			$this->servicos_concessionaria = $sql->fetchALL();
		}

		return $this->servicos_concessionaria;
	}
	...
	public function add($id_company, $Parametros)
	{
		...
		try {
			...
			if (isset($Parametros['etapas'])) {
				$this->setEtapas($Parametros, $id, $id_company);
			}
	...
	public function setServico($Parametros, $id_concessionaria, $id_company)
	{
		...
		if (isset($Parametros)) {
			if (count($Parametros) > 0) {
				for ($q = 0; $q < count($Parametros['servico']); $q++) {

					$sql = $this->db->prepare("INSERT INTO concessionaria_servico (id_concessionaria,id_servico)
							VALUES (:id_concessionaria, :id_servico)
							");
					$sql->bindValue(":id_servico", $Parametros['servico'][$q]);
					$sql->bindValue(":id_concessionaria", $id_concessionaria);
					$sql->execute();
				}
			}
